...
const trips: NextPage<Props> = (props: Props) => {
	...
	useEffect(() => {
		...
		(async () => {
			const res = await api.get('dash/trips?hours=48');
	...
	const elements = trips?.map(trip => ({
		...
		clientStatus: { text: <span className={tripStatusColors[trip.client_status as keyof typeof tripStatusColors] + ' rounded-lg px-2 py-1'}>{trip.clientStatus}</span>, searchText: trip.clientStatus, textToCopy: trip.clientStatus },
		driver: { text: `${trip.driver}\n${trip.driver_phone}`, textToCopy: trip.driver_phone },
		driverStatus: { text: <span className={tripStatusColors[trip.driver_status as keyof typeof tripStatusColors] + ' rounded-lg px-2 py-1'}>{trip.driverStatus}</span>, searchText: trip.driverStatus, textToCopy: trip.driverStatus },
	...
	const searchKeys = ['client', 'clientStatus', 'driver', 'driverStatus', 'from', 'to', 'date'];
		...
		<>
			...
			<Modal modal="!w-3/4" show={showModal} setShow={setShowModal}>
				<div className="w-full h-full flex flex-col">
					...
					<p className="text-center mt-2 font-bold">{selectedTrip?.created_at.toLocaleString('fr').replace(' ', '\n')}</p>
